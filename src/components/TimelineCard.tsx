...
export const TimelineCard: React.FC<{ entry: TimelineEntry }> = ({ entry }) => {
  ...
  return (
    ...
    >
      <img src={AVATAR_URL} alt="avatar" style={{ width: 48, height: 48, borderRadius: '50%' }} />
      <div style={{ flex: 1 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <span style={{ fontWeight: 700, color: '#222', fontSize: 18 }}>{USERNAME}</span>
          <span style={{ color: '#888', fontSize: 13 }}>{dateStr}</span>
        </div>
        <div style={{ margin: '6px 0 0 0', fontSize: 15, color: '#333' }}>
          <span style={{ fontWeight: 600, color: '#646cff' }}>Score: {entry.score}</span>
        </div>
        {entry.note && (
          <div style={{
            margin: '14px 0 0 0',
